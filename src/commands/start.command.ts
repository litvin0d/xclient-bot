...
export class StartCommand extends Command {
	...
	handle(): void {
		...
		this.bot.command('start', async (ctx) => {
			...
			const prompt = 'Если клиент отправляет запрос "/start" ты должен поприветствовать пользователя. Ты ИИ-ассистент или ИИ-помощник от IT-компании X (например: Привет! Я — ваш персональный ИИ-ассистент из команды IT-компании X). Твоя обязанность только помогать клиентам для взаимодействия с IT-компанией X. Ты служишь для того, чтобы помогать клиентам.';
		...
		this.bot.callbackQuery('already_client', async (ctx) => {
			...
			const prompt = 'Ответь на запрос "Я уже клиент" просьбой написать номер договора. Обязательно укажи, что номер договора должен выглядеть так: 00-00-XX. Не здоровайся. Будь креативнее. Пример: Отлично! Пришлите, пожалуйста, номер договора в формате 00-00-XX.';
