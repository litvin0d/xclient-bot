...
export class MessageCommand extends Command {
	...
	handle(): void {
		this.bot.on('message', async (ctx) => {
			...
			const prompt = 'Клиент может прислать номер договора (номер выглядит примерно так: 24-24-DEV, 22-22-SUP, 33-334-SEO, 26-02-PPS, 28-29-HOST), если договор был прислан, то ты должен поблагодарить клиента за это (например: Благодарю за номер договора.). Запрещается после того как клиент отправил номер договора здороваться с клиентом. Если клиент не прислал номер документа, то обязательно попроси его прислать номер документа.';
